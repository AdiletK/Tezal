...
public interface OrderMaterialRepository extends JpaRepository<OrderMaterial, Long> {
    @Query("select new kg.tezal.tezal_back.model.OrderMaterialModel(order_material.id, order_material.orders.id, order_material.numberOf, order_material.sumOf, order_material.rawMaterial.id, order_material.rawMaterial.name, order_material.rawMaterial.volume, order_material.rawMaterial.image) FROM OrderMaterial order_material WHERE order_material.orders.id = :id  ORDER BY  order_material.orders.id ASC")
    List<OrderMaterialModel> findAllByOrderId(@Param("id") Long id);

    @Query("select new kg.tezal.tezal_back.model.OrderMaterialModel(order_material.id, order_material.orders.id, order_material.numberOf, order_material.sumOf, order_material.rawMaterial.id, order_material.rawMaterial.name, order_material.rawMaterial.volume, order_material.rawMaterial.image) FROM OrderMaterial order_material WHERE order_material.id = :id ")
