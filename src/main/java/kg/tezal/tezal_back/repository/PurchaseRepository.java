...
public interface PurchaseRepository extends JpaRepository<Purchase, Long> {
    @Query("select new kg.tezal.tezal_back.model.PurchaseModel(purchase.id, purchase.count, purchase.summ, purchase.publisher, purchase.barCode, purchase.priceForOne, purchase.rawMaterial.id, purchase.rawMaterial.name, purchase.organization.id, purchase.organization.name,purchase.supplier.id, purchase.supplier.name) FROM Purchase purchase WHERE purchase.organization.id = :id  ORDER BY purchase.organization.id ASC")
