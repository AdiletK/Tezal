...
public interface RateRepository extends JpaRepository<Rate, Long> {
    @Query("select new kg.tezal.tezal_back.model.RateModel(rate.id, rate.wholesalePrice, rate.retailPrice, rate.quantityInStock, rate.rawMaterial.id, rate.rawMaterial.name,rate.rawMaterial.materialCategory.name, rate.organization.id, rate.organization.name) FROM Rate rate WHERE rate.organization.id = :id  ORDER BY rate.organization.id ASC")
    List<RateModel> findAllByOrgId(@Param("id") Long id);

    @Query("select new kg.tezal.tezal_back.model.RateModel(rate.id, rate.wholesalePrice, rate.retailPrice, rate.quantityInStock, rate.rawMaterial.id, rate.rawMaterial.name,rate.rawMaterial.materialCategory.name, rate.organization.id, rate.organization.name) " +
